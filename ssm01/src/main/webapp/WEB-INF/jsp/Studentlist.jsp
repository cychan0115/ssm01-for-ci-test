...
<table border=1 width="200" align="center" style="text-align:center">
		...
		<c:forEach var="item" items="${xxx}" >
			...
			<td>${item.age }</td>
			<td>${item.sex }</td>
		<td><form action = "/ssm02/student/selectStudentByID"><input type="hidden" name="id" value=${item.id}>
			<input type = "submit" value = "修改"></form></td>
			<td><form action = "/ssm02/student/delStu"><input type="hidden" name="id" value=${item.id}>
			<input type = "submit" value = "删除"></form></td>
		</tr>
		</c:forEach>
	</table> 
	<a href="/ssm02/student/addStu">添加</a>
